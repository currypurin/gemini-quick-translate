...
## 概要
英語テキストを選択するだけで即座に翻訳結果をストリーミング表示するChrome拡張です。ページ内に小さなポップアップが現れ、Gemini APIを利用して自然な日本語訳を届けます。翻訳モデルはポップアップから切り替え可能で、翻訳履歴の閲覧もポップアップから行えます。
...
- 翻訳モデルをGemini 2.5 Flash LiteとGemini 3.1 Flash Lite (Preview)から選択可能
...
## 設定
- **利用設定タブ**: ポップアップ内のタブでAPIキーの保存、翻訳モデルの選択（Gemini 2.5 Flash Lite / Gemini 3.1 Flash Lite Preview）、拡張機能のオン・オフ、翻訳履歴とその表示件数の調整が行えます。APIキーは保存後に入力欄がクリアされるため、必要に応じて別途メモしてください。
