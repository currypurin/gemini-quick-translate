import type { TranslateTone, TranslationHistoryItem } from '../shared/messages.js';
...
export async function addTranslationToHistory(item: TranslationHistoryItem): Promise<void> {
  const result = await chrome.storage.local.get([TRANSLATION_HISTORY_KEY]);
  const history: TranslationHistoryItem[] = Array.isArray(result[TRANSLATION_HISTORY_KEY])
...
export async function getHistoryDisplayCount(): Promise<number> {
